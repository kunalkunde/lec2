### Paired data graphs for scrutiny of local versus global  
### performance over multiple calendar month intervals
library(ggpmisc)
library(reshape2)
library(scales)
library(tidyverse)
...
nMths <- seq(from = 1, to = 12, by = 1)

plotLclGlblCompBarGraphs <- function(numMths, listOfDataSets){
...
graphTitle <- sprintf("Worst return over %d calendar months, local vs. global, 31-Dec-1999 through 15-Dec-2021",numMths)
ggplot(melted_dataForPlot,aes(x=Country_Date,y=value))+geom_bar(aes(fill=variable),stat="identity",position="dodge") +scale_x_discrete(limits=unique(melted_dataForPlot$Country_Date))+xlab("National bellwether index") +ylab("Return") +labs(title = graphTitle) +theme(axis.text.x = element_text(angle = 90, vjust = 0, hjust=0)) +scale_y_continuous(labels = scales::percent)+geom_text(aes(label = format(scales::percent(value, accuracy=.9, trim = FALSE),nsmall=2), vjust = -0.5), fontface="bold", size=3,color="black")+ theme(legend.position="bottom")
}

containerForPlots <- map(nMths, ~plotLclGlblCompBarGraphs(.x, listOfDataSets = listOfDataFrames))
