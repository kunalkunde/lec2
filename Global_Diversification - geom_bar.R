### Paired data graphs to see lcl-vs-glbl performance across multiple periods
library(ggpmisc)
library(reshape2)
library(scales)
library(tidyverse)
...
melted_dataForPlot <- melt(dataForPlot, id = "Country_Date")
ggplot(melted_dataForPlot,aes(x=Country_Date,y=value))+geom_bar(aes(fill=variable),stat="identity",position="dodge") +scale_x_discrete(limits=unique(melted_dataForPlot$Country_Index))+xlab("National bellwether index") +ylab("Return") +labs(title = "Worst return over N calendar months, local vs. global, 31-Dec-1999 through 15-Dec-2021") +theme(axis.text.x = element_text(angle = 90, vjust = 0, hjust=0)) +scale_y_continuous(labels = scales::percent)+geom_text(aes(label = format(scales::percent(value, accuracy=.9, trim = FALSE),nsmall=2), vjust = -0.5), fontface="bold", size=3,color="black")+ theme(legend.position="bottom")
# the section of code "+scale_x_discrete(limits=..." prevents sorting of the labels for the X-axis
# the section of code "+theme(axis.text.x = element_text(angle=..." tilts labels on the X-axis
# the section of code "+scale_y_continuous(labels = scales::percent)" renders percentage labels for the Y-axis
# the section of code "+geom_text(aes(label = format(scales::percent(value, accuracy=.9, trim = FALSE),nsmall=2), vjust = -0.5), fontface="bold", size=3,color="black")" formats data labels